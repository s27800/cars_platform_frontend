...
import { getPendingProposals, resolveProposal } from '../dataProposals/api';
import { Card, Spinner, Button, Badge, Pagination, Modal, TextArea } from '../../shared/components/ui';
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mb-5 text-sm">
          ...
          <span className="px-2.5 py-1 bg-neutral-100 dark:bg-neutral-700 rounded-md text-xs font-semibold uppercase tracking-wide text-neutral-600 dark:text-neutral-300">
            {getProposalCategoryLabel(proposal.category, t)}
          </span>
        </div>
...
const AdminProposalsPage = () => {
  ...
  const {
    ...
    queryKey: ['admin', 'pendingProposals', page, pageSize],
  ...
  const resolveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'pendingProposals'] });
