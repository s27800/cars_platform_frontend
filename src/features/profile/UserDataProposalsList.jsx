...
import { getUserDataProposals } from './api';
import { Card, Spinner, Button, Badge, Pagination } from '../../shared/components/ui';
import { getProposalCategoryLabel } from '../dataProposals';
import { formatDate } from '../../shared/utils/helpers';
import { PROPOSALS_PAGE_SIZE, STALE_TIME } from '../../shared/utils/constants';


// Data corrections this user proposed, with their moderation status
const UserDataProposalsList = () => {
  const { t } = useTranslation('profile');
  const [page, setPage] = useState(0);
  const pageSize = PROPOSALS_PAGE_SIZE;
  ...
  const {
    ...
    queryKey: ['user', 'proposals', page, pageSize],
  ...
  return (
    <div>
      <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
        {t('cars:proposals.submitted', { count: totalElements })}
      </p>
...
const getStatusConfig = (status, t) => {
  switch (status) {
    case 'APPROVED':
      return {
        variant: 'success',
        label: t('cars:proposals.status.approved'),
        icon: IoCheckmarkCircleOutline,
      };
    case 'REJECTED':
      return {
        variant: 'danger',
        label: t('cars:proposals.status.rejected'),
    ...
    default:
      return {
        variant: 'warning',
        label: t('cars:proposals.status.pending'),
        icon: IoTimeOutline,
      };
  }
};
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">
            {getProposalCategoryLabel(proposal.category, tCars)}
          </span>
          <p className="text-sm text-neutral-500 dark:text-neutral-400">
        ...
        {proposal.comment && (
          <div className="mb-3">
            <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-1">{tCars('proposals.yourComment')}:</p>
        ...
        {proposal.proposedValues && Object.keys(proposal.proposedValues).length > 0 && (
          <div className="mb-3">
            <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-1">{tCars('proposals.proposedChanges')}:</p>
        ...
        {proposal.resolvedAt && (
          <p className="mt-3 text-xs text-neutral-500 dark:text-neutral-400">
            {proposal.status === 'APPROVED'
              ? tCars('proposals.approvedOn', { date: formatDate(proposal.resolvedAt) })
              : tCars('proposals.rejectedOn', { date: formatDate(proposal.resolvedAt) })}
          </p>
        )}
      </div>
    </Card>
