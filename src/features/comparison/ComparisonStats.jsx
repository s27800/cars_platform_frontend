...
import { getAverageRatings } from '../reviews/api';
import { getAverageConsumption } from '../fuelReports/api';
...
const ComparisonStats = ({ carIds = [] }) => {
  ...
  const ratingsQueries = useQueries({
    queries: carIds.map(id => ({
      queryKey: ['reviews', 'averageRatings', id],
  ...
  const consumptionQueries = useQueries({
    queries: carIds.map(id => ({
      queryKey: ['fuelReports', 'averageConsumption', id],
